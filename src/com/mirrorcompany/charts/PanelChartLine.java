...
public class PanelChartLine extends javax.swing.JPanel {
    ...
    private final List<LineChart> list;
    ...
    protected void paintComponent(Graphics grphcs) {
        if (list.size() > 2){
            ...
            for (LineChart d : list){
                double location = d.getValue() * height / max;
                location = getHeight() - location;
                p.lineTo(x, location);
                x += space;
            }
            p.lineTo(x - space, getHeight() - margin);
            GradientPaint gra = new GradientPaint(0, margin, new Color(25, 69, 185), 0, getHeight(),new Color(93, 232, 246));
            g2.setPaint(gra);
            g2.fill(p);
            drawPoint(g2);
        }
    ...
    public void addItem(LineChart data){
        list.add(data);
        repaint();
    }
